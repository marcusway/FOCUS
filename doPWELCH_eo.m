...
for j = 1:nargin
    ...
    file = fullfile(subDir,txtFolder,[settings.FILE_PREFIX, subDir, 'eor.txt']); %determines name of file from subjid
    ...
    for region = 1:length(settings.REGIONS)
        ...
        save(fullfile(subDir,matFolder,[subDir, settings.AVG_PSD_FOLDER_SUFFIX],[subDir,'_PSDeo', settings.REGIONS{region},'_mean']), 'PSD');
        csvwrite(fullfile(subDir,txtFolder,[subDir, settings.AVG_PSD_FOLDER_SUFFIX],[subDir,'_PSDeo', settings.REGIONS{region}, '_mean.txt']),PSD);
        ...
        save(fullfile(subDir,matFolder,[subDir settings.IND_PSD_FOLDER_SUFFIX],[subDir, '_PSDeo' settings.REGIONS{region}]), 'Pxx_matrix');
        
        %open a text file for the individual electrode psds.  
        outfile = fopen(fullfile(subDir,txtFolder,[subDir settings.IND_PSD_FOLDER_SUFFIX],[subDir 'PSDeo' settings.REGIONS{region} '_ind.txt']),'w');
