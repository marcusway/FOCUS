function [PxxMatrix,Pxx, F] = looppwelch(segment,sensors,sampling_rate, settings)
...
len_pxx             = ceil((settings.NFFT+1)/2);                 % Length of the pwelch psd estimate output
...
for i=1:length(sensors)
    ...
    if sampling_rate == 500 % Downsample data sampled at 500Hz
        data = downsample(data,sampling_rate/settings.SAMPLING_RATE);
    end
    
    
    % Store each channel's PSD as a column in PSD matrix
    [PxxMatrix(:,i),F]=pwelch(data,settings.WINDOW,[],settings.NFFT,settings.SAMPLING_RATE);
