function FOCUS_doPWELCH_ec(varargin)
...
for j = 1:nargin
    ...
    file = fullfile(subDir,txtFolder,['ADHD_', subDir, 'ecr.txt']); %determines name of file from subjid
    ...
    for region = 1:length(FOCUS_regions)
        ...
        save(fullfile(subDir,matFolder,[subDir, '_avg_psds'],[subDir,'_PSDec', FOCUS_regions{region},'_mean']), 'PSD');
        csvwrite(fullfile(subDir,txtFolder,[subDir, '_avg_psds'],[subDir,'_PSDec', FOCUS_regions{region}, '_mean.txt']),PSD);
        ...
        save(fullfile(subDir,matFolder,[subDir '_ind_electrode_psds'],[subDir, '_PSDec' FOCUS_regions{region}]), 'Pxx_matrix');
        
        %open a text file for the individual electrode psds.  
        outfile = fopen(fullfile(subDir,txtFolder,[subDir '_ind_electrode_psds'],[subDir 'PSDec' FOCUS_regions{region} '_ind.txt']),'w');
