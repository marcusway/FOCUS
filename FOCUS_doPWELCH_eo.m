function FOCUS_doPWELCH_eo(varargin)
...
% Function changed to use the new FOCUS_looppwelch function
...
for j = 1:nargin
    ...
    [subDir, matFolder, txtFolder] = FOCUS_organize(varargin{j});
    
    % load the data using get_big_data function, which works for big files
    file = fullfile(subDir,txtFolder,['ADHD_', subDir, 'eor.txt']); %determines name of file from subjid

    try
    [data, ~, ~] = get_big_data(file); %import the file
    catch me
        problem = me.message;
        disp(['MatLab is having trouble reading in the text file, ADHD_', subDir, 'eor.txt.  Make sure that file is in the subject''s directory in the textfiles folder']);
        return
    end
    
    % Get the sampling rate from file
    load(fullfile(subDir, matFolder, ['ADHD_', subDir, '.fhp.flp.s.cr.ref.mat']), 'samplingRate');
    ...
    for region = 1:length(FOCUS_regions)
        
        % use FOCUS_looppwelch to get averages and a matrix with individual
        % electrode PSDs as columns
        [Pxx_matrix, PSD, F] = FOCUS_looppwelch_ind(data, ROI.(FOCUS_regions{region}),samplingRate);
        
        % save the regional averaged PSDs
        save(fullfile(subDir,matFolder,[subDir, '_avg_psds'],[subDir,'_PSDeo', FOCUS_regions{region},'_mean']), 'PSD');
        csvwrite(fullfile(subDir,txtFolder,[subDir, '_avg_psds'],[subDir,'_PSDeo', FOCUS_regions{region}, '_mean.txt']),PSD);
        ...
        save(fullfile(subDir,matFolder,[subDir '_ind_electrode_psds'],[subDir, '_PSDeo' FOCUS_regions{region}]), 'Pxx_matrix');
        
        %open a text file for the individual electrode psds.  
        outfile = fopen(fullfile(subDir,txtFolder,[subDir '_ind_electrode_psds'],[subDir 'PSDeo' FOCUS_regions{region} '_ind.txt']),'w');
        
        
        % Write a header row so we can identify individual channels
        % explicitly
        for electrode = 1:length(ROI.(FOCUS_regions{region}))
            fprintf(outfile,'%s\t',num2str(ROI.(FOCUS_regions{region})(electrode)));    
        end
